...
def read_scores():
    if len(sys.argv) > 1: return parse(" ".join(sys.argv[1:]))
    if os.getenv("SCORES"): return parse(os.getenv("SCORES"))
    if os.path.isfile("scores.txt"): return parse(open("scores.txt").read())
